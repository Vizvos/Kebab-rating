...
export default {
    async fetch(request: Request, env: any, ctx: any) {
        ...
        if (!isDbConnected) {
            // Použijeme MONGO_URI z Cloudflare environment vars (env) pokd existují, jinak lokální process.env z Node
            const mongoUri = env.MONGO_URI || process.env.MONGO_URI;
            if (mongoUri) {
                // Budeme potřebovat předělat config.ts na dynamic env později, prozatím ho updatneme napřímo
            }
            try {
